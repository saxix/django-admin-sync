...
class ReceiveMixin(BaseSyncMixin):
    def _receive(self, request: HttpRequest) -> "ReceivingResult":
        try:
            ...
            return ReceivingResult(message="success", size=len(request.body), records=len(data), details="", code=200)
        except DeserializationError as e:
            logger.exception(e)
            return ReceivingResult(message="error", size=len(request.body), records=0, details="", code=500)
