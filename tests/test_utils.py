...
from admin_sync.utils import (ForeignKeysCollector, get_client_ip,
                              is_logged_to_remote, remote_reverse, render,
                              set_cookie, unwrap, wraps)
from django.http import HttpResponse
from freezegun import freeze_time

DATA = '{"data": "%5B%7B%22model%22%3A%20%22auth.user%22%2C%20%22fields%22%3A%20%7B%22' \
...
def test_set_cookie(expire):
    ...
    assert response.cookies['test'] == {'comment': '',
                                        'domain': '',
...
@pytest.mark.parametrize("key,value", [("HTTP_X_ORIGINAL_FORWARDED_FOR", "127.0.0.11"),
